    ...
    <nav className="sticky top-0 z-50 w-full bg-teal-700 text-white shadow-md">
      ...
      */}
      <div className="max-w-7xl mx-auto px-2 py-0 md:px-8 flex h-[60px] items-center gap-2">
        ...
        <div className="hidden shrink-0 items-center space-x-8 md:flex">
          {links.map((link) => (
            <Link
              key={link.name}
              href={link.path}
              className="text-gray-600 hover:text-white transition font-medium"
            >
              {link.name}
            </Link>
          ))}
          <Link
            href="/pro"
            className="px-4 py-1.5 bg-teal-700 border border-[#63FF79]/30 text-[#63FF79] font-bold rounded-full text-xs hover:scale-105 transition-transform shadow-sm"
          >
            GO PRO
          </Link>
        </div>
        ...
        <Link
          href="/pro"
          className="flex md:hidden items-center justify-center shrink-0 px-3 py-1 bg-teal-700 border border-[#63FF79]/30 text-[#63FF79] font-bold tracking-wide rounded-full text-[10px] shadow-sm"
        >
          GO PRO
        </Link>
      </div>
